...
app.post('/getPostComments', auth, async (req, res) => {
  try {
...
app.get('/getSuggestedUsers', auth, async (req, res) => {
  try {
...
app.get('/getFollowing', auth, async (req, res) => {
  let user = await User.findOne({ _id: req.user.id });
  if (!user)
    return res.send(
      `Something went wrong. Cannot get users for user with id ${req.user.id}.`
    );
...
app.get('/getFollowing/:username', async (req, res) => {
  ...
  if (!user)
    return res.send(
      `Something went wrong. Cannot get users for ${req.params.username}.`
    );
...
app.put('/follow/:username', auth, async (req, res) => {
  if (req.user.id) {
...
app.put('/likePost', auth, async (req, res) => {
  try {
...
app.post('/likeComment', auth, async (req, res) => {
  try {
...
app.post('/likeSubcomment', auth, async (req, res) => {
  try {
...
app.post('/commentTheComment', auth, async (req, res) => {
  let comment = req.body.comment;
  comment = comment.trim();
  comment = comment
